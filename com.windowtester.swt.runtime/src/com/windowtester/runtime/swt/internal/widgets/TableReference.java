 ...
 */
public class TableReference extends CompositeReference<Table> {
	...
	public TableItemReference[] getItems() {
		return displayRef.execute(new Callable<TableItemReference[]>() {
			public TableItemReference[] call() throws Exception {
				return asReferencesOfType(widget.getItems(), TableItemReference.class);
			}
		});
	}
	
	public TableColumnReference[] getColumns() {
		return displayRef.execute(new Callable<TableColumnReference[]>() {
			public TableColumnReference[] call() throws Exception {
				return asReferencesOfType(widget.getColumns(), TableColumnReference.class);
			}
		});
	}
...
//		for (TableItemReference item: getItems())
//			item.accept(visitor);
//		for (TableColumnReference item: getColumns())
//			item.accept(visitor);
...
//	}
	...
	protected void setChildren(ChildSet children) {
		children.add(getChildControls()).add(getItems()).add(getColumns());
	}
