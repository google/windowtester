...
abstract class Tab {
	...
	void getValue() {
		...
		for (int i = 0; i < controls.length; i++) {
			try {
				java.lang.reflect.Method method = controls[i].getClass().getMethod(methodName, (Class[])null);
				Object result = method.invoke(controls[i], (Object[])null);
				if (result == null) {
					getText.append("null");
	...
	Class getReturnType(String methodRoot) {
		...
		Control[] controls = getExampleWidgets();
		try {
			java.lang.reflect.Method method = controls[0].getClass().getMethod(methodName, (Class[])null);
